...
public class EmoUtil {
    ...
    public static final Drawable getDisplayDrawable(Context context, int index) {
        String text = (index >= 0 && index < displayEntries.size() ?
                displayEntries.get(index).text : null);
        return text == null ? null : getDrawable(context, text);
    }

	public static final String getDisplayText(int index) {
		return index >= 0 && index < displayEntries.size() ? displayEntries
				.get(index).text : null;
	}
	...
	private static Pattern makePattern() {
		...
		sb.append(patternOfDefault());
		sb.append("|");
		sb.append(patternOfGame());

		return Pattern.compile(sb.toString());
	}

    private static String patternOfGame() {
		return "/\\[.{1,5}\\]";
	}
    ...
    private static String patternOfDefault() {
		...
		for (Entry entry : defaultEntries) {
			StringBuilder sb = sbs[entry.text.length() - 2];
		...
		StringBuilder sb = new StringBuilder();
		for (int index = sbs.length - 1; index >= 0; index--) {
			if (sb.length() > 0) {
				sb.append("|");
			}
	...
	private static final void load(Context context, String path) {
        try {
            for (String xml : context.getAssets().list(path)) {
                if (xml.endsWith(XML)) {
                    new EntryLoader().load(context, path + "/" + xml);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private static class EntryLoader extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            ...
            if (localName.equals("Catalog")) {
                ...
                Entry entry = new Entry(tag, ASSET + "/" + catalog + "/" + fileName);

                text2entry.put(entry.text, entry);
                if (catalog.equals("default")) {
                    defaultEntries.add(entry);
                    displayEntries.add(entry);
                } else if (catalog.equals("compatible")) {
                    defaultEntries.add(entry);
                }
            }
        }
